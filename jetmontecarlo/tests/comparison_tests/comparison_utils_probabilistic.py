...
from jetmontecarlo.tests.partonshower_tests.pre_and_crit.precrit_se_groomed_test_utils import *
...
# Zero bin weights:
###########################################
def precriticalEmissionWeight_zerobin(z_pre, theta_crit,
                                      z_c, jet_type='quark',
                                      fixedcoupling=True, epsilon=None):
    """Provides a weight function to integrate on the sampled phase space"""
    assert 0 < epsilon < 1, "Weight requires valid cutoff for zero bin."
    zero_bin_weights = np.exp(-2. * CR(jet_type) * alpha_fixed/np.pi
                              * np.log(R0/theta_crit)
                              * np.log(z_c/epsilon)
                              ) * (z_pre == 0)
    weights = precriticalEmissionWeight(z_pre, theta_crit, z_c, jet_type,\
                                        fixedcoupling) * (z_pre > 0)
    weights = np.nan_to_num(weights)

    # Messing around with zero bin weights
    zero_bin_weights = zero_bin_weights

    return zero_bin_weights + weights
...
def get_mc_pre_and_crit(crit_sampler, pre_sampler,
    ...
    # Adjusting generated observables for zero binning
    if BIN_SPACE == 'log':
        for i, theta in enumerate(theta_crit):
            cdf_epsilon = np.exp(-2. * CR(JET_TYPE) * alpha_fixed/np.pi
                                 * np.log(R0/theta_crit[i])
                                 * np.log(Z_CUT/pre_sampler.epsilon)
                                 )
            if random.random() < cdf_epsilon:
                z_pre[i] = 0.
    ...
    weights = (
        criticalEmissionWeight(z_crit, theta_crit,
                               ...
                               fixedcoupling=FIXED_COUPLING)
        *
        precriticalEmissionWeight_zerobin(z_pre, theta_crit,
                                          Z_CUT, JET_TYPE,
                                          fixedcoupling=FIXED_COUPLING,
                                          epsilon=pre_sampler.epsilon)
        )
    # Weights, binned observables, and area
    INTEGRATOR.setBins(NUM_BINS, obs, BIN_SPACE)
    ...
    # Adjusting jacobians for zero binning
    jacs = np.where(z_pre > 0, jacs,
        np.array(crit_sampler.jacobians)/pre_sampler.area,
        )
...
def get_mc_crit_and_sub(crit_sampler, sub_sampler,
    ...
    # Adjusting jacobians for zero binnning
    jacs = np.where(z_pre == 0, np.ones(len(jacs)), jacs)
...
def get_mc_all_emissions(crit_sampler, pre_sampler, sub_sampler,
    ...
    # Adjusting jacobians for zero binning
    jacs = np.where(z_pre == 0,
                    np.array(crit_sampler.jacobians)
                    * np.array(sub_sampler.jacobians),
                    jacs)
...
if __name__ == '__main__':
    for ieps in range(len(EPSILONS)):
        ...
        if COMPARE_ALL:
            compare_all(CRIT_SAMPLERS[ieps], PRE_SAMPLERS[ieps],
                        SUB_SAMPLERS[ieps],
                        integrate_out_pre=True,
                        integrate_out_sub=True)
            compare_all(CRIT_SAMPLERS[ieps], PRE_SAMPLERS[ieps],
