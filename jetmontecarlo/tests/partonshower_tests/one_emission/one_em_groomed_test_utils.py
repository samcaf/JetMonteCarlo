...
# Parameters:
# Shower cutoff with fixed coupling
FC_SHOWER_CUTOFF = 1e-50
# Shower cutoff with running coupling
RC_SHOWER_CUTOFF = MU_NP
...
def angularity_split_crit(ang_init, z_cut, beta,
                          jet_type, acc='LL', veto=False):
...
def angularity_shower_groomed_crit(parton, ang_init,
                                   z_cut, beta, f,
                                   jet_type, jet,
                                   acc='LL', veto=False):
    ...
    if not veto:
        continue_shower = (
            ang_init > (
                FC_SHOWER_CUTOFF*(acc == 'LL')
                + RC_SHOWER_CUTOFF*(acc in ['MLL', 'ME'])
                )
            or 'crit' not in types
            )

    if continue_shower:
        ang_final, z, theta = angularity_split_crit(ang_init, f*z_cut, beta,
                                                    jet_type, acc, veto=veto)
        ...
        angularity_shower_groomed_crit(d2, ang_final,
                                       z_cut, beta, f,
                                       jet_type, jet, acc,
                                       veto=veto)
...
# -----------------------------------------
def gen_jets_groomed_crit(num_events, z_cut, beta, f,
                          jet_type='quark',
                          radius=1., acc='LL', veto=False):
    ...
    for _ in range(int(num_events)):
        ...
        mother.type = 'mother'
        angularity_shower_groomed_crit(mother, ang_init,
                                       z_cut, beta, f,
                                       jet_type, jet,
                                       acc=acc, veto=veto)
