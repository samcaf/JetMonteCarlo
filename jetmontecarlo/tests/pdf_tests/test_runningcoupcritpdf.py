    ...
    for jet_type in ['quark', 'gluon']:
        ...
        for izcut in range(len(zcuts)):
            ...
            for j in range(len(zs_pdf)):
                ...
                label = (r"$z=$" + str(z_pdf))
                axes[0].plot(pnts,
                            critPDFAnalytic(z_pdf, pnts, zc,jet_type=jet_type),
                            **style_dashed, label=label,
                            color=compcolors[(j,'light')])
            # Labelling
            labelLines(axes[0].get_lines(), xvals = [.1,.1,.18,.33])
            ...
            for j in range(len(zs_pdf)):
                ...
                PDF  = ( splittingFn(z_pdf, jet_type, 'MLL')
                        * alpha_s(z_pdf,xs) / (np.pi * xs)
                        * np.exp(-integral) )
                yerr = ( splittingFn(z_pdf, jet_type, 'MLL')
                        * alpha_s(z_pdf,xs) / (np.pi * xs)
...
def test_CritLogPDF():
    ...
    for jet_type in ['quark', 'gluon']:
        for izcut in range(len(zcuts)):
            if savePlots and not showPlots:
                filename = (jet_type + "_rc_logpdf_test_"
                            + str(izcut) + ".pdf")
                pdffile = matplotlib.backends.backend_pdf.PdfPages(filename)
            zc = zcuts[izcut]
            zs_pdf = zlist[zc]
