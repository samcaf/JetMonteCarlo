...
#SBATCH -c 6
#SBATCH --mem=0
#SBATCH -o logs/rc_mll-%j.out
#SBATCH -e logs/rc_mll-%j.err
...
sed -i "s/NUM_MC_EVENTS = .*/NUM_MC_EVENTS = int(5e5)/" examples/params.py
...
sed -i "s/NUM_RAD_BINS = .*/NUM_RAD_BINS = int(5e2)/" examples/params.py
sed -i "s/NUM_SPLITFN_BINS = .*/NUM_SPLITFN_BINS = int(5e2)/" examples/params.py
