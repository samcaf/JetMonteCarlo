...
from examples.sudakov_comparisons.sudakov_utils import\
    sudakov_inverse_transforms, splitting_functions
...
def compare_crit(beta=BETA, plot_approx=False):
    ...
    for i, z_cut in enumerate(Z_CUT_PLOT):
        ...
        ps_vals = load_partonshower_samples('rss',
                               n_emissions='1',
                               emission_type='crit',
                               z_cuts=[z_cut], betas=[beta],
                               f_soft=F_SOFT)[z_cut][beta]
        plot_shower_pdf_cdf(ps_vals,
                            axes_pdf, axes_cdf,
...
def compare_ungroomed():
    ...
    for icol, beta in enumerate(BETAS):
        ...
        ps_vals = load_partonshower_samples('raw',
                               n_emissions='1',
                               betas=[beta])
        plot_shower_pdf_cdf(ps_vals,
                            axes_pdf, axes_cdf,
