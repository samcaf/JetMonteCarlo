#!/bin/bash
#SBATCH --job-name function_generation
#SBATCH --exclusive
#SBATCH -c 10
#SBATCH --mem=0
#SBATCH -o function_generation-%j.out
#SBATCH -e function_generation-%j.err
...
sed -i "s/OBS_ACC = .*/OBS_ACC = 'MLL'/" examples/params.py
sed -i "s/SPLITFN_ACC = .*/SPLITFN_ACC = 'MLL'/" examples/params.py
...
# ============================
sed -i "s/NUM_MC_EVENTS = .*/NUM_MC_EVENTS = 1e7/" examples/params.py
sed -i "s/NUM_RAD_BINS = .*/NUM_RAD_BINS = 5e3/" examples/params.py
sed -i "s/NUM_SPLITFN_BINS = .*/NUM_SPLITFN_BINS = 5e3/" examples/params.py
