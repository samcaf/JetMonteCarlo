...
def test_CritLinSudakov():
    ...
    for jetType in ['quark', 'gluon']:
        if savePlots and not showPlots:
            filename = jetType+"_fc_linsud_test.pdf"
        ...
        for ibeta in range(len(betas)):
            ...
            fig, axes = aestheticfig(xlabel=xlabels[ibeta],
                        ...
                        ylim  = (.7, 1.1),
                        xlim  = (0, .5),
                        title = 'Critical '+jetType+' Sudakov factor, '
                                + r'fixed $\alpha_s$',
                        showdate   = False,
                        ratio_plot = False)
            ...
            for i in range(len(zcuts)):
                ...
                label = (r"$z_c=$" + str(zc))
                axes[0].plot(pnts,
                            critSudakov_fc_LL(pnts, zc,beta, type=jetType),
                            **style_dashed,
            ...
            for i in range(len(zcuts)):
                ...
                weights = criticalEmissionWeight(z, theta, zc, jetType,
                                                fixedcoupling=True)
...
def test_CritLogSudakov():
    ...
    for jetType in ['quark', 'gluon']:
        for ibeta in range(len(betas)):
            ...
            if savePlots and not showPlots:
                filename = jetType+"_fc_logsud_test_"+str(beta)+".pdf"
                pdffile = matplotlib.backends.backend_pdf.PdfPages(filename)
            ...
            for izc in range(len(zcuts)):
                ...
                for ieps in range(len(epsilons)):
                    ...
                    analytic = critSudakov_fc_LL(xs, zc,beta,type=jetType)
