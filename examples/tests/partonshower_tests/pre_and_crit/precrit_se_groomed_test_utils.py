...
def angularity_split_precrit_se(ang_init, z_cut, beta,
                                jet_type, acc='LL'):
    ...
    while not accept_emission:
        ...
        if z > z_cut:
            accept_emission = True
        elif not has_precrit:
            # Generate a pre-critial emission with this value of z
            accept_emission = False
            z_cut = z_cut-z
            ang_init = ang_final
        else:
            # Generate a critical emission by acceptance-rejection
            accept_emission = False
    return ang_final, z, theta, z_cut

def angularity_shower_groomed_veto_precrit_se(parton, ang_init,
                                              z_cut, beta, f,
                                              jet_type, jet,
                                              acc='LL'):
    ...
    if continue_shower:
        ang_final, z, theta, z_cut = angularity_split_precrit_se(ang_init,
                                                                 f*z_cut, beta,
                                                                 jet_type, acc)
...
# -----------------------------------------
def gen_jets_groomed_precrit_se(num_events, z_cut, beta, f,
                                jet_type='quark',
                                radius=1., acc='LL'):
    ...
    for _ in range(int(num_events)):
        ...
        mother.type = 'mother'
        angularity_shower_groomed_veto_precrit_se(mother, ang_init,
                                                  z_cut, beta, f,
                                                  jet_type, jet,
                                                  acc=acc)
